// Package settings provides the theme picker settings screen.
package settings
...
// Model is the Bubble Tea model for the settings/theme screen.
type Model struct {
	cursor int
	width  int
	height int
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "enter", " ":
			selected := theme.All[m.cursor]
			name := selected.Name
			return m, func() tea.Msg {
				return common.ThemeChangedMsg{ThemeName: name}
			}
		}
	}
	return m, nil
...
func (m Model) View() string {
	...
	b.WriteString(common.RenderTitle("Theme", m.width))
	b.WriteString("\n")
